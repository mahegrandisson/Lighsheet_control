...
samples_per_cycle = 1  # Nombre d'échantillons par cycle (1 échantillon à chaque impulsion)
...
for _ in range(40):
    tmp=time.time()
    times.append(tmp)
    task.WriteDigitalLines(samples_per_cycle, True, 10.0, PyDAQmx.DAQmx_Val_GroupByChannel,
                           np.array([1], dtype=np.uint8), ct.byref(samples_written), None)


    task.WriteDigitalLines(samples_per_cycle, True, 10.0, PyDAQmx.DAQmx_Val_GroupByChannel,
                           np.array([0], dtype=np.uint8),
                           ct.byref(samples_written), None)
