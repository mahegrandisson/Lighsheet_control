...
def generate_sin_wave(start_value, end_value, frequency, duration: float, sampling_rate=1000):
...
def fast_fluo(frequency, duration, sine_start: float, sine_stop: float, sample_number_per_sine_period=10000):

    step_cam = 1/(2*frequency)

    taskCAM = Task()
    taskCAM.CreateCOPulseChanTime("/Dev1/ctr1", "", PyDAQmx.DAQmx_Val_Seconds, PyDAQmx.DAQmx_Val_Low, 0, step_cam, step_cam)
    ...
    task_ms.CreateCOPulseChanTime("/Dev1/ctr0", "", PyDAQmx.DAQmx_Val_Seconds, PyDAQmx.DAQmx_Val_Low, 0, step_val, step_val)
    ...
    taskG.WriteAnalogF64(len(wave_2), False, 10.0, PyDAQmx.DAQmx_Val_GroupByScanNumber, dataG, None, None)
    taskG.StartTask()
    taskCAM.StartTask()
    task_ms.StartTask()
    tmp = time.time()
...
if __name__=="__main__":
    ...
    sine_start = -0.04
    sine_stop = 0.05
    fast_fluo(frequency, duration, sine_start, sine_stop, sample_number_per_sine_period=10000)
